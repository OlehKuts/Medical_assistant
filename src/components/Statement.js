...
import { finalVersionCreator } from "../utils/statementUtils";
import { childTypeList, maleNames } from "../initialData/statementData";
import {
  maxilloSortedList,
  otoSortedList,
} from "../initialData/codeHelperData";
import { Block } from "./Block";
import { maleChecker, randomizer } from "../utils/statementUtils";

export const Statement = ({ onHideNavbar, doctors }) => {
  ...
  return (
    <div className="statement">
      {showForm && <h5>Заповніть форму для отримання довідки</h5>}
      {showForm && (
        <div className="dateForm">
          <form onSubmit={onSubmit}>
            <div className="checkboxLine">
              <div className="preCheckbox">ЛОР-патологія</div>
              <div>
                <input
            ...
            <div className="formLine">
              {" "}
              <label>Початок </label>
              <input value={startDate} onChange={onStartDateChange} />
            </div>
            <div className="formLine">
              {" "}
            ...
            <div className="formLine">
              <select value={doctor} onChange={onDoctorChange}>
                {doctors.map(({ title, id, value }) => (
                  <option disabled={!value} value={value} key={id}>
                    {title}
                  </option>
                ))}
              </select>
